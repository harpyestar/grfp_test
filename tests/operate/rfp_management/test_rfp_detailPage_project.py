...
class TestRFPDetailPageProject:
    ...
    @pytest.mark.parametrize(
        "test_data",
        add_notes_cases,
        ids=[tc["case_id"] for tc in add_notes_cases]
    )
    ...
    @pytest.mark.parametrize(
        "test_data",
        expand_collapse_cases,
        ids=[tc["case_id"] for tc in expand_collapse_cases]
    )
    @allure.title("报价详情页 - 内部跟进备注展开/收起功能验证")
    @allure.description("""
    测试: Operate 角色在报价详情页中验证内部跟进备注的展开/收起功能
    ...
    7. 选择第一个酒店
    8. 点击 Detail 按钮打开详情页弹窗
    9. 在详情页中，验证备注展开按钮是否存在
    10. 若存在，点击展开按钮，验证收起按钮出现

    预期结果: 备注展开按钮存在，点击后收起按钮可见
    """)
    async def test_verify_notes_expand_collapse(self, page_module, operate_user, test_data):
        """
        参数化的详情页内部跟进备注展开/收起功能测试（登录状态复用）
        ...
        try:
            with allure.step(f"用例: {test_data['description']}"):
                ...
                with allure.step("验证备注展开按钮是否存在"):
                    expand_exists = await detail_page.verify_expand_button_exists(detail_popup)
                    assert expand_exists, "备注展开按钮不存在，但预期应存在（项目已包含备注内容）"

                with allure.step("点击展开按钮并验证收起按钮"):
                    retract_exists = await detail_page.click_expand_and_verify_retract(detail_popup)
                    assert retract_exists, "点击展开后，收起按钮未出现"
